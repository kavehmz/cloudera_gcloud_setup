...
NODE01IP=$(gcloud compute instances describe $GCLOUD_ZONE-cloudera-node1 --zone $GCLOUD_ZONE|grep natIP|cut -d' ' -f 6)
...
ssh kaveh@$NODE01IP 'impala-shell -q "invalidate metadata"'
